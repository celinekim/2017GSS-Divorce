...
raw_data <- read_csv("AAC08avn.csv")
...
gss <- gss %>% 
  ...
  rename(CASEID = caseid,
         ...
         age_of_spouse_at_first_marriage = aprma1c,
         common_law_before_marriage = ma1_150,
         end_type_first_marriage = ma1_170
  ) 

#### Clean up ####
gss <- gss %>% 
  select(CASEID, age_at_first_marriage, age_of_spouse_at_first_marriage,
         common_law_before_marriage,
         end_type_first_marriage) %>%
  mutate(age_at_first_marriage = ifelse(age_at_first_marriage=="99.6" | 
                                          age_at_first_marriage=="99.7" |
                                          age_at_first_marriage=="99.8" |
                                          age_at_first_marriage=="99.9",
                                        "NA", age_at_first_marriage)) %>%
  mutate(age_of_spouse_at_first_marriage = ifelse(age_of_spouse_at_first_marriage=="99.6" |
                                                    age_of_spouse_at_first_marriage=="99.7" |
                                                    age_of_spouse_at_first_marriage=="99.8" |
                                                    age_of_spouse_at_first_marriage=="99.9",
                                        "NA", age_of_spouse_at_first_marriage)) %>%
  mutate(common_law_before_marriage = ifelse(common_law_before_marriage=="1", 1, 0)) %>%
  # If first marriage ended in separation and then divorce or divorce, end_type_first_marriage is 1. Anything else is 0.
  mutate(end_type_first_marriage = ifelse(end_type_first_marriage=="1" |
                                            end_type_first_marriage=="4",
                                          1,0)) %>%
  select(CASEID, age_at_first_marriage, age_of_spouse_at_first_marriage,
         common_law_before_marriage,
         end_type_first_marriage)
